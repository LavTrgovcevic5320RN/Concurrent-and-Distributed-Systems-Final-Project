...
//    }
    ...
    public void init(WelcomeMessage welcomeMsg) {
        ...
        this.friends = new CopyOnWriteArrayList<>(welcomeMsg.getFriends());
...
//        MessageUtil.sendMessage(agm);
    ...
    public boolean isFriend(int i) {
        for(ServentInfo friend : friends) {
            if(friend.getListenerPort() == i) {
                return true;
            }
        }
        return false;
    ...
    public void addFriend(String ip, int port) {
        ServentInfo newFriend = new ServentInfo(ip, port);
        if(!friends.contains(newFriend)) {
            friends.add(newFriend);
        } else {
            AppConfig.timestampedStandardPrint("Friend " + ip + ":" + port + " already exists.");
        }
    }

    public void addFile(MyFile file, String requesterIpAddress, int requesterPort) {
        if(!files.contains(file)) {
            files.add(file);
            AppConfig.timestampedStandardPrint("File " + file.getPath() + " added.");

            int nextNodePort = AppConfig.chordState.getNextNodePort();

            Message addFileMessage = new AddFileMessage(AppConfig.myServentInfo.getListenerPort(),
                    nextNodePort, "", requesterPort, requesterIpAddress, file);
            MessageUtil.sendMessage(addFileMessage);

        } else {
            AppConfig.timestampedStandardPrint("File " + file.getPath() + " already exists.");
        }
    }

    public void removeFile(MyFile file) {
        if(!files.contains(file)) {
            AppConfig.timestampedStandardPrint("File " + file.getPath() + " not found.");
            return;
        }

        MyFile fileToRemove = null;
        for(MyFile f : files) {
            if(f.equals(file)) {
                fileToRemove = f;
                break;
        ...
        if(fileToRemove != null) {
            ...
            AppConfig.timestampedStandardPrint("File " + file.getPath() + " removed.");

            int nextNodePort = AppConfig.chordState.getNextNodePort();
            Message removeFileMessage = new RemoveFileMessage(AppConfig.myServentInfo.getListenerPort(),
                    nextNodePort, "", fileToRemove);
